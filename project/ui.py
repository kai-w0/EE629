...
class Ui:
    ...
    def build_loading(self):
        self.screen.addstr(6, 19, 'Enjoy your music，loading...', curses.color_pair(1))
        self.screen.refresh()        
    ...
    def build_menu(self, datatype, title, datalist, offset, index, step):
        ...
        if len(datalist) == 0:
            self.screen.addstr(8, 19, 'Nothing here -，-')
            ...
            if datatype == 'main':
                ...
                self.screen.addstr(20, 6, 'NetEase-MusicBox based on Python')
    ...
    def build_search(self, stype):
        ...
        if stype == 'songs':
            song_name = self.get_param('Search for songs：')
            try:
            ...
            artist_name = self.get_param('Search for artists：')
            try:
            ...
            artist_name = self.get_param('Search for albums：')
            try:
            ...
            artist_name = self.get_param('Search for new lists：')
            try:
    ...
    def build_search_menu(self):
        self.screen.move(4,1)
        self.screen.clrtobot()
    	self.screen.addstr(8, 19, 'Select search type:', curses.color_pair(1))
    	self.screen.addstr(10,19, '[1] Songs')
    	self.screen.addstr(11,19, '[2] Artists')
    	self.screen.addstr(12,19, '[3] Albums')
    	self.screen.addstr(13,19, '[4] Selected Playlists')
    	self.screen.addstr(16,19, 'Please insert number:', curses.color_pair(2))
    	self.screen.refresh()
    	x = self.screen.getch()
    	return x

    def build_login(self):
        info = self.get_param('Please sign in， e.g: john@163.com 123456')
        account = info.split(' ')
        if len(account) != 2:
    ...
    def build_login_error(self):
        ...
        self.screen.addstr(11,19, '[2] Try it later')
        self.screen.addstr(14,19, 'Please insert number:', curses.color_pair(2))
        self.screen.refresh()
        x = self.screen.getch()
        return x
